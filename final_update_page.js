...
content = content.replace(
    /<div className="print-wrapper" style={{([^}]+)maxWidth:\s*'210mm'([^}]*)}}>/g,
    `<div className="print-wrapper" style={{$1maxWidth: estimate.templateType === 'PROJECT_BREAKDOWN' ? '297mm' : '210mm'$2}}>`
);
...
// 3. Fix inline dimensions of a4-document using regex
content = content.replace(
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    console.error("Could not find start or end index for Table.");
}

fs.writeFileSync(file, content, 'utf8');
console.log("REPLACED EVERYTHING EXACTLY AND ROBUSTLY.");
