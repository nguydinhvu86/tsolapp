...
export function EditDispatchClient({ dispatch, templates, customers, products = [] }: {
    ...
    const [selectedCustomer, setSelectedCustomer] = useState<string>(dispatch.customerId);
    const [selectedTemplate, setSelectedTemplate] = useState<string>(dispatch.templateId);
    const [content, setContent] = useState(dispatch.content);

    // Parse variables safely
    const initialVars = dispatch.variables ? JSON.parse(dispatch.variables) : {};
    const [variables, setVariables] = useState<any>(initialVars);
    const [isSaving, setIsSaving] = useState(false);
    const [previewContent, setPreviewContent] = useState(dispatch.content);
    ...
    useEffect(() => {
        ...
        setVariables((prev: any) => {
            ...
            uniqueVars.forEach(v => {
                // If it's already set from initial DB load, keep it, otherwise derive
                if (newVars[v] === undefined) {
                    ...
                    // Update customer details if customer changed but variable was pre-filled
                    if (v === 'TEN_KHACH_HANG' || v === 'TEN_DOANH_NGHIEP') newVars[v] = customer.name;
                    else if (v === 'MA_SO_THUE') newVars[v] = customer.taxCode || '';
                    else if (v === 'DIA_CHI_KHACH_HANG' || v === 'DIA_CHI') newVars[v] = customer.address || '';
                    else if (v === 'EMAIL') newVars[v] = customer.email || '';
