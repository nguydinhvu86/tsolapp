...
export default function PrintSalesInvoiceClient({ invoice, companyInfo, settings }: any) {
    ...
    const handleExportPDF = () => {
        ...
        document.title = `Hoa_Don_${invoice.code}`;
        window.print();
    ...
    return (
        <div className="print-wrapper" style={{
            ...
            <div className="no-print" style={{
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                    ...
                    <button onClick={handlePrint} style={{
                        ...
                        <Printer size={18} /> In Hóa Đơn (Ctrl + P)
                    </button>
                </div>
            </div>
            ...
            <div className="print-container" style={{
                ...
                <Watermark settings={settings} documentType="SALES_INVOICE" />
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #1e293b', paddingBottom: '1.5rem', marginBottom: '2rem' }}>
                    <div style={{ flex: 1 }}>
                        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, margin: '0 0 0.5rem 0', textTransform: 'uppercase', color: '#0f172a' }}>
                            {companyInfo?.name || 'TÊN CÔNG TY CHƯA CẬP NHẬT'}
                        </h2>
                        <div style={{ fontSize: '0.95rem', lineHeight: '1.5', color: '#334155' }}>
                            {companyInfo?.address && <div><strong>Địa chỉ:</strong> {companyInfo.address}</div>}
                            {companyInfo?.phone && <div><strong>Điện thoại:</strong> {companyInfo.phone}</div>}
                            {companyInfo?.email && <div><strong>Email:</strong> {companyInfo.email}</div>}
                            {companyInfo?.taxCode && <div><strong>Mã số thuế:</strong> {companyInfo.taxCode}</div>}
                        </div>
                    </div>
                    {companyInfo?.logo && (
                        <div style={{ marginLeft: '2rem' }}>
                            <img src={companyInfo.logo} alt="Company Logo" style={{ maxHeight: '80px', maxWidth: '200px', objectFit: 'contain' }} />
                        </div>
                    )}
                </div>
                ...
                <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
                    <h1 style={{ fontSize: '2.25rem', fontWeight: 800, margin: '0 0 0.5rem 0', color: '#0f172a' }}>HÓA ĐƠN BÁN HÀNG</h1>
                    <i style={{ fontSize: '1rem', color: '#475569' }}>Số: {invoice.code} | Ngày lập: {formatDate(invoice.date)}</i>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem', fontSize: '1.05rem', lineHeight: '1.6' }}>
                    <div style={{ flex: 1, paddingRight: '1rem' }}>
                        ...
                        {invoice.customer?.company && <div><strong>Công ty:</strong> {invoice.customer?.company}</div>}
                    ...
                    <div style={{ flex: 1, paddingLeft: '1rem', textAlign: 'right' }}>
                        <h3 style={{ fontSize: '1.1rem', fontWeight: 700, borderBottom: '1px dotted #ccc', display: 'inline-block', marginBottom: '0.5rem' }}>ĐIỀU KIỆN & THANH TOÁN</h3>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '2rem', fontSize: '1rem' }}>
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td style={{ border: '1px solid #cbd5e1', padding: '12px 8px', textAlign: 'right', fontWeight: 800, fontSize: '1.2rem', color: '#0f172a' }}>{formatMoney(invoice.totalAmount)}</td>
                        </tr>
                    </tfoot>
                </table>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 2rem', marginTop: '4rem' }}>
                    <div style={{ textAlign: 'center' }}>
                        <strong style={{ display: 'block', fontSize: '1.1rem' }}>NGƯỜI MUA HÀNG</strong>
                        <i style={{ fontSize: '0.9rem', color: '#64748b' }}>(Ký và ghi rõ họ tên)</i>
                        <div style={{ height: '100px' }}></div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                        <strong style={{ display: 'block', fontSize: '1.1rem' }}>NGƯỜI LẬP BIỂU</strong>
                        <i style={{ fontSize: '0.9rem', color: '#64748b' }}>(Ký và ghi rõ họ tên)</i>
                        <div style={{ height: '100px' }}></div>
