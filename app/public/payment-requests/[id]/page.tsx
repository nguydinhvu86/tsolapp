...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
    ...
    const title = paymentRequest?.title ? `${paymentRequest.title} | T-SOLUTION` : 'Đề Nghị Thanh Toán | T-SOLUTION';
    const description = paymentRequest?.customer?.name 
        ? `Đề nghị thanh toán trực tuyến cho khách hàng ${paymentRequest.customer.name} - T-SOLUTION`
        : 'Chi tiết đề nghị thanh toán trực tuyến - T-SOLUTION';
...
export default async function PublicPaymentRequestPage({ params }: { params: { id: string } }) {
    const paymentRequest = await prisma.paymentRequest.findUnique({
        where: { id: params.id },
        include: { customer: true }
    });
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f8fafc', padding: '2rem 1rem', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            ...
            <div className="no-print" style={{ maxWidth: '210mm', margin: '0 auto 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', padding: '1rem 1.5rem', borderRadius: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                <div>
                    <h1 style={{ margin: 0, fontSize: '1.125rem', fontWeight: 700, color: '#0f172a' }}>{paymentRequest.title}</h1>
                    <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.875rem', color: '#64748b' }}>Gửi đến: <strong>{paymentRequest.customer.name}</strong></p>
                </div>
                <PrintButton />
            </div>
            ...
            <div className="a4-document print-wrapper" style={{ position: 'relative', width: '100%', maxWidth: '210mm', minHeight: '297mm', padding: '15mm 20mm', margin: '0 auto', background: 'white', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)', boxSizing: 'border-box' }}>
                <Watermark settings={settingsMap} documentType="PAYMENT_REQUEST" />
