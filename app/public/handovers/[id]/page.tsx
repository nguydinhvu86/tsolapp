...
export async function generateMetadata({ params }: { params: { id: string } }) {
    ...
    return { title: handover?.title || "Biên Bản Bàn Giao" };
}

export default async function PublicHandoverPage({ params }: { params: { id: string } }) {
    const handover = await prisma.handover.findUnique({
        where: { id: params.id },
    ...
    return (
        <div style={{ minHeight: '100vh', background: '#f8fafc', padding: '2rem 1rem', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            ...
            <div className="no-print" style={{ maxWidth: '210mm', margin: '0 auto 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', padding: '1rem 1.5rem', borderRadius: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                <div>
                    <h1 style={{ margin: 0, fontSize: '1.125rem', fontWeight: 700, color: '#0f172a' }}>{handover.title}</h1>
                    <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.875rem', color: '#64748b' }}>Giao cho: <strong>{handover.customer.name}</strong></p>
                </div>
                <PrintButton />
            </div>
