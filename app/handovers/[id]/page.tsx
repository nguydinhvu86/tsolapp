...
export default async function HandoverPrintPage({ params }: { params: { id: string } }) {
    ...
    if (!handover) return notFound();

    const tasks = await prisma.task.findMany({
        where: { handoverId: params.id },
    ...
    return (
        <div className="handover-layout-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 350px', gap: '1.5rem', alignItems: 'start', paddingBottom: '3rem' }}>
            {/* Left Area: Main Content */}
            <div className="handover-layout-main" style={{ overflow: 'hidden' }}>
        ...
        @media print {
          ...
          .handover-layout-grid { display: block !important; }
          .handover-layout-main { overflow: visible !important; width: 100% !important; max-width: 100% !important; }
      ...
      `}} />
                        ...
                        <div style={{ position: 'relative' }}>
                            <Watermark settings={settingsMap} documentType="HANDOVER" />
                            <div
                                ...
                                dangerouslySetInnerHTML={{ __html: formatCurrencyInHtml(handover.content) }}
                            />
                        </div>
                    </div>
                </div>
            ...
            <div className="no-print" style={{ position: 'sticky', top: '1rem' }}>
                <TaskPanel initialTasks={tasks} users={users} entityType="HANDOVER" entityId={params.id} />
            </div>
        </div>
    );
}
