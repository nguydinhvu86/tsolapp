...
export function NewHandoverClient({ templates, customers, products, preselectedCustomerId }: { templates: HandoverTemplate[], customers: Customer[], products: Product[], preselectedCustomerId?: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!selectedTemplate || !selectedCustomer) {
            alert('Vui lòng chọn đầy đủ Mẫu biên bản và Khách hàng');
            return;
        }

        try {
            await createHandover({
                title: `Biên bản - ${selectedCustomer.name} - ${new Date().toLocaleDateString('vi-VN')}`,
        ...
        <div className="flex" style={{ gap: '2rem', alignItems: 'flex-start' }}>
                ...
                <Card>
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-2">
                            <label style={{ fontWeight: 500, fontSize: '0.875rem' }}>Chọn Mẫu Biên Bản</label>
                ...
                {selectedTemplate && (
                    <Card>
                        ...
                        <div style={{ marginTop: '2rem' }}>
                            <Button onClick={handleSubmit} className="w-full">Tạo Biên Bản</Button>
                        </div>
                    </Card>
                )}
            </div>
            ...
            <div style={{ flex: 1.5 }}>
                <Card style={{ position: 'sticky', top: '80px', minHeight: '600px' }}>
                    <h3 style={{ marginBottom: '1rem', borderBottom: '1px solid var(--border)', paddingBottom: '1rem' }}>Xem trước Biên bản</h3>
                    {previewContent ? (
                        <div
                            className="ql-editor"
