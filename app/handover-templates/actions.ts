...
export async function createHandoverTemplate(data: { name: string, description?: string, content: string }) {
    await prisma.handoverTemplate.create({ data });
    revalidatePath('/Handover-templates');
}

export async function updateHandoverTemplate(id: string, data: { name: string, description?: string, content: string }) {
