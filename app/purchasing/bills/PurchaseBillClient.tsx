...
import { Plus, Search, Eye, Trash2, Calendar, FileText, FileDown, CheckCircle, ArrowUpDown, Edit2, XCircle, AlertTriangle, X, Clock, GripVertical, ChevronUp, ChevronDown } from 'lucide-react';
...
export function PurchaseBillClient({ initialBills, suppliers, orders, warehouses, products, projects }: { initialBills: any[], suppliers: any[], orders: any[], warehouses: any[], products: any[], projects?: any[] }) {
    ...
    const moveItem = (index: number, direction: 'up' | 'down') => {
        ...
        if (targetIndex < 0 || targetIndex >= billItems.length) return;
        const newItems = [...billItems];
                                            ...
                                            <table className="w-full min-w-[600px] text-xs mb-3 bg-white text-left">
                                                ...
                                                <tfoot>
                                                    <tr className="bg-slate-50/80 border-t border-slate-200">
                                                        <td colSpan={5} className="p-2.5 text-right font-medium text-slate-600 text-xs">{t('purchaseBills.subTotal')}:</td>
                                                        <td className="p-2.5 text-right font-semibold text-slate-800 text-xs">{formatMoney(calculateSubTotal())}</td>
                                                        <td className="p-2.5"></td>
                                                    </tr>
                                                    <tr className="bg-slate-50/80">
                                                        <td colSpan={5} className="p-2.5 text-right font-medium text-slate-600 text-xs">{t('purchaseBills.totalTax')}:</td>
                                                    ...
                                                    <tr className="bg-emerald-50/50 border-t border-emerald-200">
                                                        <td colSpan={5} className="p-2.5 text-right font-bold text-slate-800 text-xs">{t('purchaseBills.grandTotal')}:</td>
