...
interface DocumentManagersPanelProps {
    documentId: string;
    managers: any[];
    users: any[];
    currentUserRole: string;
    onAssign: (documentId: string, userIds: string[]) => Promise<any>;
    onRemove: (documentId: string, userId: string) => Promise<any>;
}

export function DocumentManagersPanel({ documentId, managers = [], users = [], currentUserRole, onAssign, onRemove }: DocumentManagersPanelProps) {
    ...
    // Users not yet managing this document
    ...
    const handleAddManager = async (userId: string) => {
        ...
        try {
            await onAssign(documentId, [userId]);
    ...
    const handleRemoveManager = async (userId: string) => {
        ...
        setIsSubmitting(true);
        try {
            await onRemove(documentId, userId);
        } catch (e: any) {
            alert(e.message);
        } finally {
            setIsSubmitting(false);
        ...
        <Card style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div>
                        <h3 style={{ margin: 0, fontSize: '1.125rem', fontWeight: 600, color: 'var(--text-main)' }}>Người Phụ Trách</h3>
                        <p style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text-muted)' }}>{managers.length} người đang phụ trách văn bản này</p>
                    </div>
                </div>
                {canEdit && (
