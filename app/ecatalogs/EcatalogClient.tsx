...
import { submitEcatalog, updateEcatalogStatus, deleteEcatalog, updateEcatalog, convertEstimateToInvoice, convertEstimateToOrder } from './actions';
import { formatMoney, formatDate } from '@/lib/utils/formatters';
import { TagDisplay } from '@/app/components/ui/TagDisplay';
import { AvatarImage } from '@/app/components/ui/AvatarImage';

export default function EcatalogClient({ initialEstimates, customers, products, leads, projects, nextCode, initialAction, initialCustomerId, initialLeadId, initialProjectId, users, currentUserId, isAdminOrManager }: any) {
        ...
        if (typeof window !== 'undefined') {
            ...
            if (editId && estimates.length > 0) {
                ...
                if (estToEdit && estToEdit.status === 'DRAFT') {
                    ...
                    window.history.replaceState({}, '', '/ecatalogs');
                }
            }
        }
    ...
    const handleSave = async () => {
        ...
        let res;
        if (formData.id) {
            res = await updateEcatalog(formData.id, formData);
        } else {
            res = await submitEcatalog('system', formData); // 'system' or real user id if context available
        }
    ...
    const handleStatusChange = async (id: string, newStatus: string) => {
        setActionModal({
            ...
            action: async () => {
                const res = await updateEcatalogStatus(id, newStatus);
                if (res.success) {
    ...
    const handleDelete = async (id: string) => {
        setActionModal({
            ...
            action: async () => {
                const res = await deleteEcatalog(id);
                if (res.success) {
        ...
        <>
            <Card className="p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <h2 className="text-xl font-semibold">Ecatalog ERP</h2>
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        router.push(`/ecatalogs?${params.toString()}`);
                                    }}
                                >
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 mb-6 flex gap-4 items-center flex-wrap">
                    ...
                    <div className="shrink-0">
                        <button onClick={handleOpenCreate} className="btn-primary w-full sm:w-auto mt-4 sm:mt-0 font-semibold h-[40px] px-6 text-[13px] shadow-sm flex items-center justify-center transition-all hover:-translate-y-0.5" title="Tạo mới Ecatalog">
                        ...
                        <div className="shrink-0 min-w-[200px] w-full sm:w-auto">
                            <select
                                ...
                                onChange={(e) => {
                                    ...
                                    window.location.href = `/ecatalogs?${params.toString()}`;
                                }}
                            >
                            ...
                            return (
                                <tr key={est.id} className={`border-t border-gray-100 ${isExpired ? 'bg-gray-50/50' : ''}`}>
                                    <td className="py-3 items-center gap-2 flex">
                                        ...
                                        <Link href={`/ecatalogs/${est.id}`} className={`font-semibold hover:text-primary hover:underline transition-colors block ${isExpired ? 'text-gray-500' : 'text-gray-800'}`}>
                                            {est.code}
                                        </Link>
                                    </td>
                                    <td className="py-3">
                                    ...
                                    <td className="py-3 text-right">
                                        <div className="flex justify-end items-center gap-2">
                                            <div className="flex items-center gap-1 mr-1">
                                                ...
                                                <Link href={`/ecatalogs/${est.id}`} title={t('estimates.viewDetails')} className="p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 transition-colors block">
                                                    <Eye size={15} />
                                                </Link>
                                            ...
                                            {est.status === 'EXPIRED' && (
                                                <Button variant="secondary" onClick={() => handleCopy(est)} className="text-emerald-700 bg-emerald-50 border-emerald-200 hover:bg-emerald-100 hover:border-emerald-300 px-3 flex-shrink-0 py-1.5 text-xs font-semibold shadow-sm transition-all rounded-md" title="Tạo một Ecatalog mới sao chép toàn bộ dữ liệu này">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-gray-700 mb-1.5">Mẫu Ecatalog</label>
            ...
            <Modal isOpen={!!convertModalId} onClose={() => !isConverting && setConvertModalId(null)} title="Xác nhận Lên Hóa Đơn">
                <div className="p-6" style={{ fontFamily: 'Inter, sans-serif' }}>
                    <p className="text-gray-700 text-[15px] mb-6 leading-relaxed">
                        Bạn có chắc chắn muốn chuyển dữ liệu từ Ecatalog này thành <strong>Hóa Đơn</strong> không? Các thông tin chi tiết sẽ được tự động sao chép sang Hóa Đơn mới.
                    </p>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '2rem', padding: '1rem', borderRadius: '0.75rem', backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}>
                        ...
                        <div style={{ fontSize: '0.875rem', color: '#1e3a8a', lineHeight: 1.625, marginTop: '0.125rem' }}>
                            ...
                            Ecatalog này sẽ tự động chuyển thành trạng thái <strong style={{ backgroundColor: '#dbeafe', padding: '0.125rem 0.375rem', borderRadius: '0.25rem', color: '#1d4ed8', fontWeight: 700 }}>"Đã Chốt"</strong> sau quá trình khởi tạo thành công.
                        </div>
                    </div>
            ...
            <Modal isOpen={!!convertOrderModalId} onClose={() => !isConvertingOrder && setConvertOrderModalId(null)} title="Xác nhận Lên Đơn Đặt Hàng">
                <div className="p-6" style={{ fontFamily: 'Inter, sans-serif' }}>
                    <p className="text-gray-700 text-[15px] mb-6 leading-relaxed">
                        Bạn có chắc chắn muốn chuyển dữ liệu từ Ecatalog này thành <strong>Đơn Đặt Hàng</strong> không? Các thông tin chi tiết sẽ được tự động sao chép sang Đơn Đặt Hàng mới.
                    </p>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '2rem', padding: '1rem', borderRadius: '0.75rem', backgroundColor: '#eff6ff', border: '1px solid #bfdbfe', boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}>
                        ...
                        <div style={{ fontSize: '0.875rem', color: '#1e3a8a', lineHeight: 1.625, marginTop: '0.125rem' }}>
                            <strong style={{ display: 'block', marginBottom: '0.25rem', fontWeight: 600 }}>Cập nhật tự động</strong>
                            Ecatalog này sẽ tự động chuyển thành trạng thái <strong style={{ backgroundColor: '#e0e7ff', padding: '0.125rem 0.375rem', borderRadius: '0.25rem', color: '#4338ca', fontWeight: 700 }}>"Đã Lên Đơn"</strong> sau quá trình khởi tạo thành công.
                        </div>
                    </div>
