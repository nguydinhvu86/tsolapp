...
import { updateEcatalogStatus, convertEstimateToInvoice, convertEstimateToOrder } from '../actions';
import { formatMoney, formatDate } from '@/lib/utils/formatters';
import { TaskPanel } from '@/app/components/tasks/TaskPanel';
import { Modal } from '@/app/components/ui/Modal';
import { EcatalogActivityLog } from '@/app/components/sales/EcatalogActivityLog';
import { SendEmailModal } from '@/app/components/ui/modals/SendEmailModal';
import { sendEstimateEmail, assignEcatalogManagers, removeEcatalogManager, cloneEcatalog } from '../actions';
...
export default function EcatalogDetailClient({ initialData, customers, products, users, emailTemplates, settings }: any) {
    ...
    const handleStatusChange = async (newStatus: string) => {
        setActionModal({
            ...
            message: `Xác nhận đổi trạng thái Ecatalog thành: ${newStatus}?`,
            action: async () => {
                const res = await updateEcatalogStatus(estimate.id, newStatus);
                if (res.success) {
    ...
    const handleClone = async () => {
        setActionModal({
            ...
            message: `Hệ thống sẽ tạo ra một Ecatalog Nháp mới với toàn bộ dữ liệu từ Ecatalog này. Tiếp tục?`,
            action: async () => {
                const res = await cloneEcatalog(estimate.id);
        ...
        <div style={{ padding: '0', maxWidth: '100%', margin: '0 auto', fontFamily: 'Inter, sans-serif' }}>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 md:mb-8">
                    ...
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.25rem' }}>
                            <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#0f172a', margin: 0, letterSpacing: '-0.025em' }}>
                                Ecatalog {estimate.code}
                            </h1>
                            {getStatusBadge(estimate.status)}
                        </div>
                        <p style={{ color: '#64748b', margin: 0, fontSize: '0.875rem' }}>Quản lý chi tiết Ecatalog và các công việc liên quan.</p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto mt-4 md:mt-0">
                    ...
                    <button
                        onClick={() => router.push(`/ecatalogs?edit=${estimate.id}`)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 mt-6">
                        ...
                        <div className="p-4 sm:p-6">
                            ...
                            {activeTab === 'managers' && (
                                <DocumentManagersPanel
                                    ...
                                    onAssign={assignEcatalogManagers}
                                    onRemove={removeEcatalogManager}
                                />
                            )}
                ...
                <div className="lg:col-span-1 flex flex-col gap-6">
                    ...
                    <EcatalogActivityLog logs={estimate.activityLogs || []} />
