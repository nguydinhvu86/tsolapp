...
conn.on('ready', () => {
    console.log('Connected to server. Uploading CustomerClient.tsx...');

    conn.sftp((err, sftp) => {
        if (err) throw err;

        const localFile = path.join(__dirname, 'app', 'customers', 'CustomerClient.tsx');
        const remoteFile = '/www/wwwroot/inside.tsol.vn/tsolapp/app/customers/CustomerClient.tsx';
