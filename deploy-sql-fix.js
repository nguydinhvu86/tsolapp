...
conn.on('ready', () => {
    ...
    conn.sftp((err, sftp) => {
        ...
        console.log('SFTP session opened. Transmitting migrate-fix.sql...');
        sftp.fastPut('migrate-fix.sql', '/www/wwwroot/inside.tsol.vn/tsolapp/migrate-fix.sql', (err) => {
            if (err) {
            ...
            const cmd = `cd /www/wwwroot/inside.tsol.vn/tsolapp && ` +
                ...
                `npx prisma db execute --file migrate-fix.sql --schema prisma/schema.prisma && ` +
                `npx prisma generate && ` +
                `npm run build && ` +
                `pm2 restart all`;
