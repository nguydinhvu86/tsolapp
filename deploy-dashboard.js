...
conn.on('ready', () => {
    console.log('Connected to server. Uploading Dashboard Page fix...');
    conn.sftp((err, sftp) => {
        if (err) throw err;

        const localPath = 'C:/Users/admin/Documents/CONTRACT/app/dashboard/page.tsx';
        const remotePath = '/www/wwwroot/inside.tsol.vn/tsolapp/app/dashboard/page.tsx';

        sftp.fastPut(localPath, remotePath, (err) => {
            if (err) throw err;
            console.log(`Uploaded dashboard/page.tsx`);

            console.log('Running build on production server...');
