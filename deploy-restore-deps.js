...
conn.on('ready', () => {
    console.log('Connected to server. Uploading original package.json and restoring node_modules...');
    conn.sftp((err, sftp) => {
        if (err) throw err;

        const filesToUpload = [
            { local: 'C:/Users/admin/Documents/CONTRACT/package.json', remote: '/www/wwwroot/inside.tsol.vn/tsolapp/package.json' },
            { local: 'C:/Users/admin/Documents/CONTRACT/package-lock.json', remote: '/www/wwwroot/inside.tsol.vn/tsolapp/package-lock.json' }
        ];
        ...
        filesToUpload.forEach(file => {
            sftp.fastPut(file.local, file.remote, (err) => {
                ...
                if (uploadedCount === filesToUpload.length) {
                    console.log('Files uploaded. Running npm ci and build...');
