...
conn.on('ready', () => {
    const cmd = `cd ${APP_PATH} && export PATH=/www/server/nvm/versions/node/v24.14.0/bin:$PATH && (npx prisma migrate deploy > migrate_error.txt 2>&1 ; cat migrate_error.txt)`;
    conn.exec(cmd, (err, stream) => {
        if (err) throw err;
        let dataStr = '';
        stream.on('close', (code) => {
            fs.writeFileSync('temp/error_out.txt', dataStr);
            conn.end();
            process.exit(0);
        });
